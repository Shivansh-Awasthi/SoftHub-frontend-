...
const Ps4 = () => {
    ...
    const handleData = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_API}/api/apps/category/ps4`);
            setData(response.data.apps);
        } catch (error) {
            console.log("Error fetching PS4 ISO data: " + error);
        }
    };
        ...
        <div className='container mx-auto p-2'>
            <div className='cover mb-6'>
                <h1 className='font-medium text-3xl mb-4'>
                    PS4 ISO <span className='font-medium ml-2 text-[#8E8E8E]'>{data.length}</span>
                </h1>
            </div>
            ...
            <div className="flex flex-wrap justify-start gap-8 max-w-full">
                ...
                {data.map((ele) => (
                    <Link
                        key={ele._id}
                        to={`/${ele._id}`} // Adjust the path as necessary for your routing
