...
const MacRedux = () => {
    ...
    const { macApps } = useSelector(state => state.apps);
    ...
    const { data = [], total: totalApps = 0, loading: isLoading = false, error } = macApps;
    ...
    const hasAccess = (game) => {
        if (!game.isPaid) return true; // Free games are accessible to all
        
        const isAdmin = localStorage.getItem("role") === 'ADMIN';
        if (isAdmin) return true; // Admins have access to all games
        
        const purchasedGames = JSON.parse(localStorage.getItem("gData")) || [];
        return purchasedGames.includes(game._id); // Check if user purchased the game
    };

    // Game card component
    const GameCard = ({ game }) => {
        const gameHasAccess = hasAccess(game);
        
        return (
            <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105">
                <Link to={`/download/mac/${createSlug(game.title)}/${game._id}`}>
                    ...
                    <div className="p-4">
                        ...
                        <p className="text-sm text-gray-600 mt-1 truncate">{game.description}</p>
                    </div>
                </Link>
            </div>
        );
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-3xl font-bold text-center mb-8">Mac Games</h1>
                ...
                <>
                    ...
                    <div className="mb-6 text-center">
                        <p className="text-gray-600">
                            Showing {data.length} of {totalApps} Mac games
                        </p>
                    </div>
